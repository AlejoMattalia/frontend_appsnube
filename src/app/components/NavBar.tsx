...
export const NavBar = () => {
    ...
    return (
        <>
            <nav className='w-full min-h-16 bg-darkGray flex items-center justify-between p-5 gap-5 z-40'> {/* Added z-40 */}
                <p className='font-bold text-lg cursor-pointer' onClick={() => router.push('/')}>Productos</p>
                <div className='flex items-center gap-5'>
                    <div className='flex gap-3 flex-wrap justify-end'>
                    <p className='ml-4 text-white cursor-pointer' onClick={() => router.replace('/admin')}>Administrador</p>
                    <p className='ml-4 text-white cursor-pointer' onClick={() => router.replace('/auth/login')}>Iniciar Sesión</p>
